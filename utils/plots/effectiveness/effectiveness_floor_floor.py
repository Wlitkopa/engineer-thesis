...
for i in range(1, 4):
    ...
    if i == 2:
        regev_floor_floor_effectiveness_1_N_1[0] = 98
        regev_floor_floor_effectiveness_1_p_q[0] = 98
    ...
    plt.plot(N[:len(regev_floor_floor_effectiveness_1_p_q)], regev_floor_floor_effectiveness_1_p_q,
             label=f"Regev's algorithm - non-trivial square root of unity modulo N", color=colors[i-1][1])
    # if i == 3:
    plt.plot(N[:len(shor_effectiveness)], shor_effectiveness, label="Shor's algorithm - factorization of N", color="purple")

    plt.title(f"Comparison of effectiveness, floor_floor, type of test {type_of_test}")
...
# plt.savefig(f"./../../../images/plots/effectiveness/effectiveness_floor_floor_all.png")
